...
def _build_entities(coordinator: TapoP110DataCoordinator, subentry_id: str) -> list:
    """Build the switch entities for one device subentry."""
    return [
        TapoP110PowerSwitch(coordinator, SWITCHES[0], subentry_id),
        TapoP110AutoOffSwitch(coordinator, SWITCHES[1], subentry_id),
        TapoP110AutoUpdateSwitch(coordinator, SWITCHES[2], subentry_id),
        TapoP110PowerProtectionSwitch(coordinator, SWITCHES[3], subentry_id),
    ]
...
async def async_setup_entry(
    ...
    """Set up Tapo P110 switches, one set per device subentry (initial setup)."""
...
async def async_setup_subentry(
    ...
    """Set up Tapo P110 switches for a single device subentry (post-setup add)."""
    coordinators: dict[str, TapoP110DataCoordinator] = entry.runtime_data
